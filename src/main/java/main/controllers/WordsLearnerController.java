...
import main.services.TransferObj;
...
public class WordsLearnerController {
    ...
    public List<Word> getWordsForPlay(@RequestBody TransferObj transferObj) throws JsonProcessingException, SQLException {
        List<Word> wordForPlayList=new ArrayList<Word>();
        Word word=wordsLearnerService.getWordForRepeat(transferObj);
        if (word!=null)
            wordForPlayList.add(word);
        wordForPlayList.addAll(wordsLearnerService.getWordForPlay(transferObj));
        return wordForPlayList;
    }
}
